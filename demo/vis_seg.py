...
def img_save_and_viz_without_json(
...
def img_save_and_viz(
    ...
    output_json_file = output_path.replace(".jpg", ".json").replace(".jpeg", ".json").replace(".png", ".json")
    ...
    for label, color in zip(labels, colors):
        mask[sem_seg == label, :] = color
        region_coords = np.argwhere(sem_seg == label).tolist()
        regions[classes[label]] = region_coords
    ...
    print(f"Saved segmentation results to {output_json_file}")
